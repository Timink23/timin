...
//EVERYTHING BEFORE THIS WAS GIVEN TO US. PART OF CONFIGURING THE I2C DRIVER
...
// Get Device ID//given in i2c breif
...
uint8_t readRegister(uint8_t reg) {
    uint8_t ret;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();//starts
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (SLAVE_ADDR << 1) | WRITE_BIT, ACK_CHECK_EN);//write to ack
    i2c_master_write_byte(cmd, reg, ACK_CHECK_EN);//register to ack
    i2c_master_start(cmd);//start again
    i2c_master_write_byte(cmd, (SLAVE_ADDR << 1) | READ_BIT, ACK_CHECK_EN); //read to ack
    i2c_master_read_byte(cmd, &ret, ACK_CHECK_DIS);//adress of ret to nack
    i2c_master_stop(cmd);
...
int16_t read16(uint8_t reg)
{
    uint8_t ret1;
    uint8_t ret2;
    ret1 = readRegister(reg);//use the same function as before so i dont have to do it again
    if (reg == 41) {
        ret2 = 0;
    }
    else {
        ret2 = readRegister(reg + 1);
    }
    return (((int16_t)ret2 << 8) | ret1);
}
